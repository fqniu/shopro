<template>
	<view class="adv-box mx20 mb10">
		...
		<!-- 模板8-->
		<view class="type7 y-f" v-if="detail.style == 8">
			<view class="x-f type7-box1">
				<image class="type7-img1" @tap="jump(detail.list[0].path)" :src="detail.list[0].image" mode="aspectFill"></image>
				<image class="type7-img1" @tap="jump(detail.list[1].path)" :src="detail.list[1].image" mode="aspectFill"></image>
			</view>
			<view class="x-f type7-box2">
				<image class="type7-img2" @tap="jump(detail.list[2].path, {'type':1})" :src="detail.list[2].image" mode="aspectFill"></image>
				<image class="type7-img2" @tap="jump(detail.list[3].path, {'type':2})" :src="detail.list[3].image" mode="aspectFill"></image>
			</view>
		</view>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		jump(path, params = {}) {
			...
			this.$tools.routerTo(path, params);
		}
	}
};
</script>
