...
export default {
	...
	onLoad(query) {
		//是否有邀请人
		var inviter_id = decodeURIComponent(this.$Route.query.scene).split("=");
		var inviter_name = inviter_id[0].replace(/(^\s*)|(\s*$)/g, "");  
		if(inviter_id && inviter_name == 'inviter_id' && inviter_id[1] > 0){
			console.log(inviter_id[1]);
			uni.setStorageSync('inv		iter_id', inviter_id[1]);
		}
		// #ifdef APP-VUE
